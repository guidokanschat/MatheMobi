...
\begin{itemize}
    \item Die Ableitung $\frac{df(x)}{dx}$ beschreibt die {\bf Änderung} von $f(x)$ bei infinitesimalen Veränderungen von $x$
    \item {\bf Partielle Ableitung}: Änderung der Funktion $f(x_1,x_2,\ldots)$ bei infinitesimalen Veränderungen {\bf einer} Variablen, wobei die anderen {\bf konstant} bleiben!
\end{itemize}

    $$
    \frac{f(x_1,\ldots,x_k+\Delta x_k, \ldots, x_n) - f(x_1,\ldots, x_n)}{\Delta x_k} \xrightarrow{\Delta x_k \to 0} \frac{\partial f(x_1,\ldots,x_n)}{\partial x_k}
    $$
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Schreibweisen:
        $$
        \frac{\partial f}{\partial x_k}~~\text{oder}~~\partial_{x_k} f ~~\text{oder}~~f_{x_k}~~\text{oder}~~f_{k}
        $$
        \item Beispiele:
        \begin{eqnarray*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    \frametitle{Mehrfachableitungen}

    \begin{itemize}
        \item Wie bei Funktionen einer Variablen können wir {\bf mehrfach-Ableitungen} berechnen
        \item Jedoch sind hier alle Kombinationen von Variablen möglich!
