...
\gkerror{Warnung vor linearen Polynomen!}
...
\begin{frame}
  ...
  \item Integrierender Faktor
    \begin{gather*}
      m(t) = \exp\left(-\int_0^t s\,ds\right) = e^{-\frac{t^2}{2}}
    \end{gather*}
  \item Integral der rechten Seite
    \begin{gather*}
  ...
  \begin{columns}
    ...
    \begin{column}[t]{.49\textwidth}
      \begin{block}{GNU Octave}
        ...
        \item Über bwJupyter direkt verfügbar
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Numerische Löser sind Integrationsverfahren}
  \begin{itemize}
  \item Alle numerischen Verfahren lösen die Volterrasche Integralgleichung
    \begin{gather*}
      x(t) = x(t_0) + \int_{t_0}^t f\bigl(s, x(s)\bigr) \ds.      
    \end{gather*}
  \item Sie benötigen damit immer einen Anfangswert
  \item Sie implementieren im Kern numerische Integration
  \end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{lsode (Octave)}
  Die Funktion\footnote{Livermore Solver for Ordinary Differential Equations}
  \lstinline!lsode(@fun, x0, tinv)! löst
  \begin{gather*}
...
\begin{frame}{ode45 (Matlab, Octave)}
  ...
  \item \lstinline!x0! ist der Startwert, immer als Vektor
  \end{enumerate}
  Die Funktion gibt einen Vektor mit Zeitpunkten und einen mit
...
\begin{frame}{Einführung in Zeitschrittverfahren}
  \begin{itemize}
  \item Kenntnis der grundlegenden Methode
  \item Grundverständnis der Konvergenz der Verfahren
  \item Grenzen der Aussagekraft numerischer Lösungen
  \end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Eulersches Polygonzugverfahren}
  \begin{columns}
    \begin{column}{.58\textwidth}
      \begin{gather*}
        x_{k} = x_{k-1} + h_k f(t_{k-1},x_{k-1})
      \end{gather*}
      \begin{center}
        \includegraphics[width=.99\textwidth]{fig/euler-logistic.pdf}
      \end{center}
    ...
    \begin{column}{.38\textwidth}
      \begin{gather*}
        x_0 = 0.2,\qquad h_k \equiv h = 0.1
      \end{gather*}
      \begin{align*}
        x_0 &= 0.2\\
        x_1 &= x_0 + h f(x_0) = 0.216\\
        x_2 &= x_1 + h f(x_1) = 0.232934\\
        x_3 &= x_2 + h f(x_2) = 0.250802\\
        \vdots&\\
        x_{10} &= x_9 + h f(x_9) = 0.399697\\
        \vdots&\\
        x_{100} &= 0.9998735
      \end{align*}
    \end{column}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Verbesserung der Lösungen}
  \begin{itemize}
  \item Lösungen des Eulerverfahrens approximieren nicht sehr gut
  \item Kleinere Schrittweite $h$ führt zu besserer Approximation
  \item Die Bahnen sind nicht mehr geschlossen!
    \pause
  \item Erster Schritt des Verfahrens
    \begin{multline*}
      \vx(t_0+h) = \vx_0
      + \int_{t_0}^{t_0+h} \vf\bigl(s,\vx(s)\bigr) \,ds
      \\
      \visible<3->{
      \approx \vx_0 + h \vf(t_0,\vx_0) = \vx_1}
    \end{multline*}
  \end{itemize}
  \visible<4->{
    \begin{Denkpause}
      Wie können wir das Integral besser approximieren?
    \end{Denkpause}
}
\end{frame}
...
\begin{frame}{Das allgemeine Konzept}
  Beginnend mit dem Startwert $\vx_0$ ,an der Stelle $t_0$,
  berechne rekursiv weitere Werte der approximierten Lösung $\vx_k$ für $k=1,2,\ldots$:
  \begin{align*}
    t_k &= t_{k-1} + h_k\\
    \vx_k &= \vx_{k-1} + h_k \vF_{h_k}(t_{k-1},\vx_{k-1}).
  \end{align*}
  \begin{itemize}
  \item $h_k$ ist die \textbf{Schrittweite} im Schritt $k$
  \item $\vF_{h}(t,\vx)$ ist die \textbf{Verfahrensfunktion}
  \item Das Resultat ist eine Zerlegung des Zeitintervalls in kleine
    Intervalle der Länge $h_k$
        \begin{tikzpicture}
      \draw(0,1) --  (9,1);
      ...
      \draw[thick](9,-.1) node[anchor=north]{$t_n$} --(9,.12);
    \end{tikzpicture}
  \end{itemize}
\end{frame}

\begin{frame}{Die Wahl der Verfahrensfunktion $\vF_h(t,\vx)$}
  \begin{itemize}
  \item Wähle $\vF_{h}(t,\vx)$, so dass allgemein
    \begin{gather*}
      \vx + h \vF_{h}(t,\vx)
      \approx \vx + \int_t^{t+h} f\bigl(s,\vy(s)\bigr)\,ds,
    \end{gather*}
    wobei $\vy(s)$ Lösung der Volterraschen Integralgleichung mit
    Startwert $\vx$ zum Zeitpunkt $t$ ist.
  \item Wir können nun bessere Quadraturformeln benutzen
    \pause
  \item Achtung! Die Lösung am zweiten Quadraturpunkt hängt von der am
    ersten ab, etc.
  \item Die Simpson-Regel ist leider nicht direkt anwendbar
  \end{itemize}
\end{frame}

\begin{frame}{Beispiel: das \glqq{}klassische\grqq{} Runge-Kutta-Verfahren}
  \begin{xalignat*}2
    && \vk_1 &= \vf\left(t,\vx\right)\\
    \vy_1 &= \vx+\tfrac12 h\vk_1 &  \vk_2 &= f\left(t+\tfrac12 h,\vy_1\right)\\
    \vy_2 &= \vx+\tfrac12 h\vk_2 & \vk_3 &= f\left(t+\tfrac12 h,\vy_2\right)\\
    \vy_3 &= \vx+ h\vk_3 & \vk_4 &= f\left(t+h,\vy_3\right)\\
  \end{xalignat*}
  \vspace{-1.5cm}
  \begin{gather*}
    \vF_h(t,\vx) = h\left(\tfrac16\vk_1+\tfrac13\vk_2+\tfrac13\vk_3+\tfrac16\vk_4\right)    
  \end{gather*}
  Die Punkte $\vy_1,\dots,\vy_3$ sind Hilfskoordinaten, die ähnlich dem
  Euler-Verfahren konstruiert werden, die Verfahrensfunktion ist dann
  eine geeignete Quadraturformel der Funktionswerte an diesen Punkten.

  \pause
  Die Werte $\vk_1,\dots,\vk_4$ hei\ss{}en \textbf{Stufen}, wir haben also ein vierstufiges Verfahren
\end{frame}

\begin{frame}{Berechung mit dem Runge-Kutta-Verfahren}
  Wir wollen die Zahl $e$ als Wert $x(1)$ mit der Lösung der Aufgabe
  \begin{gather*}
    x'=x,\qquad x(0) = 1
  \end{gather*}
  berechnen. Die Schrittweite sei $h=1$

  \only<2>{
    \begin{Denkpause}
      Überlegen Sie sich, wie das mit dem Eulerschen Polygonzugverfahren und mit dem Runge-Kutta-Verfahren gehen könnte.
    \end{Denkpause}
  }
  \begin{itemize}
  \item<3-> $f(x_0) = x_0 = 1$. Daher gilt mit Euler:
    \begin{gather*}
      e = x(1) \approx x_0 + h f(x_0) = 2
    \end{gather*}
  \item<4-> Die Stufen des Runge-Kutta-Verfahrens sind mit $k_1=1$
    \begin{align*}
      k_2 = y_1 &= 1+ 1/2 = 3/2\\
      k_3 = y_2 &= 1+ 3/4 = 7/4\\
      k_4 = y_3 &= 1+7/4 = 11/4
    \end{align*}
  \item<5-> Anwendung der Formel für die Verfahrensfunktion
    \begin{gather*}
      x(1) \approx 1 + \frac16 + \frac{3}{2\cdot 3} + \frac{7}{3\cdot4}
      + \frac{11}{4 \cdot 6} = \frac{65}{24} \approx 2.70833
    \end{gather*}
  \end{itemize}
\end{frame}
...
\begin{frame}{ode45}
  Das Verfahren zur \textbf{Schrittweitensteuerung} von Dormand und Prince
  \begin{itemize}
  \item Zwei Zeitschrittverfahren mit 4 und 5 Stufen
  \item Schätze den lokalen Fehler durch die Differenz der Lösungen
  \item Wähle einen Steuerungsparameter
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Approximation und qualitative Eigenschaften}
  \begin{itemize}
  \item Die Lösung mit \lstinline!ode45! ist sehr genau
    \begin{itemize}
    \item gesteuert durch Parameter in Matlab oder Octave
    \item Der Erhaltungssatz ist mit derselben Genauigkeit erfüllt
    \end{itemize}
  \item Der Erhaltungssatz gilt \textbf{nicht} für das numerische Verfahren
    \only<2>{
      \begin{Denkpause}
        Wann kann das zu einem Problem werden?
      \end{Denkpause}
    }
  \item<3-> Bei vielen Umläufen addieren sich diese Fehler wieder auf
  \item<4-> Es werden auch spezielle numerische Verfahren mit
    Erhaltungseigenschaft entwickelt
  \end{itemize}
\end{frame}

\begin{Abschluss}
\item Welche Funktion in Matlab/Octave \glqq{}löst\grqq{} Differentialgleichungen quasi automatisch?
\item Was ist die Schrittweite und welche Auswirkung hat sie auf
  die Genauigkeit?
\item Wie wählt man die Verfahrensfunktion?
\item Wie funktioniert Schrittweitensteuerung?
\item Erfüllen numerische Verfahren Erhaltungssätze?
\end{Abschluss}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Fehler bei der Simulation}
...
\begin{frame}{Fehlerquellen bei der Simulation einer AWA}
  ...
  \only<2>{
    \begin{block}{}
      Nicht gemeint sind Programmierfehler!
    \end{block}
}
\end{frame}
...
\begin{frame}{Modellierung von Mess- und Modellfehler}
  \begin{itemize}
  \item Mess- und Rundungsfehler bei den Startwerten modellieren wir,
    indem wir für den natürlichen Prozess und das Modell verschiedene
    Startwerte $\vx_0$ und $\vy_0$ annehmen
  \item Alle anderen Modell-, Diskretisierungs- und Rechenfehler
    werden durch zwei verschiedene rechte Seiten modelliert
  \end{itemize}
  \begin{align*}
    \text{tatsächlich:}\qquad \vx(t)
    &= \vx_0 + \int_{t_0}^t \vf\bigl(s,\vx(s)\bigr)\,ds \\
    \text{Modell:}\qquad \vy(t)
    &= \vy_0 + \int_{t_0}^t \vg\bigl(s,\vy(s)\bigr)\,ds
  \end{align*}
\end{frame}
...
\begin{frame}{Fehlerdarstellung}
  \begin{gather*}
    \vy(t)-\vx(t) = \vy_0-\vx_0
    + \int_0^t \Bigl[
    \vg\bigl(s,\vy(s)\bigr) - \vf\bigl(s,\vx(s)\bigr)\Bigr] \,ds
  \end{gather*}
  \pause
  Ergänzen einer null
  \begin{multline*}
    \vy(t)-\vx(t) = \colorbox{cyan!30!white}{\(\displaystyle \vy_0-\vx_0\)}
    + \colorbox{red!40!white}{\(\displaystyle
      \int_0^t \Bigl[
      \vg\bigl(s,\vy(s)\bigr) - \vf\bigl(s,\vy(s)\bigr)\Bigr] \,ds\)}\\
    + \colorbox{yellow}{\(\displaystyle
      \int_0^t \Bigl[
    \vf\bigl(s,\vy(s)\bigr) - \vf\bigl(s,\vx(s)\bigr)\Bigr] \,ds\)}
  \end{multline*}
  \begin{enumerate}
  \item \colorbox{cyan!40!white}{Anfangswertfehler}: Genauigkeit der Messung
  \item \colorbox{red!40!white}{Modellfehler}: Genauigkeit des Modells und der Numerik
  \item \colorbox{yellow}{Fehlerfortpflanzung}: Eigenschaften von $f$
  \end{enumerate}
\end{frame}
...
%    \only<2->{z' &= 2z & z(0) &= 3.1 & z(t) &= 3.1 e^{2t}}
  ...
  \begin{alignat*}2
    \colorbox{red!40!white}{\(\displaystyle
      \int_0^t \bigl[g(y)-f(y)\bigr]\)}
    &= \int_0^t 0.31 e^{2.1s} &&= \tfrac{0.31}{2.1}\bigl(e^{2.1t}-1\bigr)
    \\
    \colorbox{yellow}{\(\displaystyle
      \int_0^t \bigl[f(y)-f(x)\bigr]\)}
    &= \int_0^t 2(3.1e^{2.1s} - 3e^{2s}) &&= \tfrac{6.2}{2.1}\bigl(e^{2.1t}-1\bigr) - 3\bigl(e^{2t}-1\bigr)
  \end{alignat*}
  \begin{columns}
...
\begin{frame}{Schranken für die Fehlerterme}
  ...
  \begin{block}{Lipschitzbedingung}
    Es gibt eine Konstante $L$, so dass für alle relevanten $x,y,t$ gilt:
    \begin{gather*}
...
\begin{frame}{Was bedeutet relevant?}
  \begin{itemize}
...
\begin{frame}
  ...
  Der Term $e^{Lt}$ wächst in der Regel schnell. Deswegen gibt es für
  viele Gleichungen angepasste Bedingungen an $f(t,\vx)$, die bessere
  Abschätzungen erlauben.
\end{frame}

\begin{Abschluss}
\item Welche Fehlerquellen sind zu berücksichtigen?
\item In welche Komponenten kann man den Fehler aufspalten?
\item Welche Bedeutung hat die Lipschitzbedingung in der Fehlerabschätzung?
\end{Abschluss}
