...
\mode<article>{Wir versuchen uns nun dem Integral anzunähern, indem wir es von unten und oben einschließen. Dazu bestimmen wir die minimale und maximale Geschwindigkeit.}
...
\mode<article>{Das ist offensichtlich eine sehr grobe Abschätzung. Wir können sie aber verbessern, wenn wir kleinere Intervalle nehmen.}
...
\mode<article>{Die Bilder zeigen exemplarisch, wie sich die Maxima und Minima immer mehr dem Graphen der Funktion annähern. Auch sehen wir, wie die Fläche des blauen Bereichs, die gerade die Differenz zwischen Unter- und Obersumme darstellt, schrumpft.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\gkerror{Hier für das Skript Zwischentexte einfügen}
...
Berechnen Sie folgende Integrale mittels partieller Integration. \textit{Hinweis:} Sie müssen diese Methoden ggf. mehrfach anwenden.
    ...
    \text{c)}\;& \int_0^1  x^2e^x\,dx \qquad \\
    \text{d)}\;& \int_0^{\pi} x^2\sin(x)\,dx \qquad &
    \text{e)}\;& \int_0^\pi \sin(x)\cos(x)\,dx \qquad
    \end{alignedat}
    \end{align*}
\end{Aufgabe}
...
\begin{columns}
    \begin{column}{.49\textwidth}
        ...
        \begin{gather*}
            ...
            =
            {\frac12\color{red}\int_0^{2a} f(t)\,dt}
        \end{gather*}
    \end{column}
    \begin{column}{.49\textwidth}
        \begin{center}
...
\label{frame:circle}
    ...
    \end{gather*}
    \pause
    Die Intervallgrenzen müssen nun so gewählt werden, dass
    \begin{gather*}
        \cos a = 0,\qquad \cos b = 1.
    \end{gather*}
    Das gilt z.B. für $a = -\frac\pi2$ und $b = 0$.
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    \frametitle<presentation>{Beispiel (Forts.)}
    Berechnen wir nun die Stammfunktion (der Sinus ist im Integrationsbereich negativ):
    \begin{multline*}
        \int_0^1 \sqrt{1-x^2}\,dx = - \int_{-\frac\pi2}^0 \sqrt{1-\cos^2 t}\,\sin t\,dt
        \\
        = \int_{-\frac\pi2}^0 \sin^2 t\,dt
        = \frac{1}2\int \bigl(1-\cos (2t)\bigr)\,dt
    \end{multline*}
    Nun können wir elementar die Stammfunktion berechnen: 
    \begin{gather*}
        f(t) = \frac{1-\cos(2t)}{2},\qquad F(t)
        = \frac t2-\frac{\sin(2t)}4
    \end{gather*}
    \pause
    und einsetzen
    \begin{gather*}
        \int_0^1 \sqrt{1-x^2}\,dx = \left[
        \frac t2-\frac{\sin(2t)}4\right]_{-\frac\pi2}^0
        = 0-0+0+\frac\pi4
    \end{gather*}
\end{frame}
...
\begin{Aufgabe}
    Berechnen Sie die Stammfunktionen folgender Integrale. Nutzen Sie ggf. die trigonometrischen Identitäten.
    \begin{align*}
    \begin{alignedat}{2}
...
\begin{frame}
    ...
    \begin{figure}[H]
      \centering
      \resizebox{0.8\linewidth}{!}{\input{fig/inverse_cuberoot.tikz}}
    \end{figure}
    \begin{itemize}
...
\begin{frame}
    ...
    \begin{figure}[H]
      \centering
      \resizebox{0.8\linewidth}{!}{\input{fig/inverse_linear.tikz}}
    \end{figure}
    \begin{itemize}
...
\mode<article>{Die Unter- und Obersummen aus der Herleitung des Integrals sind zu dieser Approximation aus zwei Gründen nicht geeignet:}
...
Das Prinzip hinter der Trapezregel ist, dass sie lineare Polynome exakt integriert und dass bereits mit dieser Anforderung die Formel eindeutig festgelegt ist. Wir wissen bereits, dass eine Gerade in der Ebene durch zwei Punkte $\bigl(x_i,f(x_i)\bigr)$ und $\bigl(x_{i+1},f(x_{i+1})\bigr)$ eindeutig bestimmt ist. Im Folgenden bestimmen wir diese Gerade, betrachten sie als Funktion, und berechnen das Integral. Die Punkte werden interpoliert durch die Gerade
    \begin{gather*}
        p(t) = f(x_i) \frac{x_{i+1}-t}{x_{i+1}-x_i}
        + f(x_{i+1}) \frac{t-x_{i}}{x_{i+1}-x_i}
    \end{gather*}
...
\mode<article>{Da hier nur ein Wert der Funktion $f$ zur Verfügung steht, um das Polynom zu bestimmen, müssen wir das Argument, warum die Formel für lineare Polynome exakt ist, abwandeln. Dazu legen wir eine Gerade mit beliebiger Steigung $\alpha$ durch den Mittelpunkt:
\begin{gather*}
...
\mode<article>{Im Plenum diskutieren wir, wie aus der tabellierten Simpsonregel die vorher präsentierte Summenformel wird.}
\gkerror{Hier ergänzen!}
...
\begin{Aufgabe}
  Benutzen Sie das zur Verfügung gestellte Jupyter Notebook mit der
...
\begin{Aufgabe}
    Berechnen Sie das Volumen $V(r)$ einer Kugel vom Radius $r$, indem Sie die Herleitung des Volumens der Einheitskugel verallgemeinern.
\end{Aufgabe}
