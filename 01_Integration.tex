...
\begin{frame}{Das Integral als Fläche unter einer Kurve}
    \begin{itemize}
        \item Die stückweise konstanten Funktionen, die die Unter- und Obersummen definieren, konvergieren gegen die Funktion $f$.
        \begin{itemize}
            \item ...zumindest, wenn $f$ stetig ist.
        \end{itemize}
8        \item Wir schließen, dass das Integral die Fläche zwischen der Funktion $f$ und der $t$-Achse beschreibt
...
\begin{Vertiefung}
    ...
    \end{gather*}
    in $n$ Teilintervalle geteilt wird. $f_i$ ist ein Funktionswert auf dem Teilintervall $[x_{i-1},x_i]$ und für die Gewichte $h_i$ gilt unabhängig von $n$ \mrwarning{Noch kurz rationalisieren, warum Gewichte hier ein passender Begriff ist}
    \begin{gather*}
        \sum_{i=1}^n h_i = b-a.
    \end{gather*}
    ...
    \begin{columns}
        ...
        \begin{block}{Gerade Funktion}
            \begin{gather*}
                f(-x) = f(x)
            \end{gather*}
            \resizebox{\columnwidth}{!}{\input{fig/even_functions.tikz}}
            \begin{gather*}
    ...
    \begin{column}{.5\textwidth}
        \begin{block}{Ungerade Funktion}
            \begin{gather*}
                f(-x) = -f(x)
            \end{gather*}
            \resizebox{\columnwidth}{!}{\input{fig/odd_functions.tikz}}
            \begin{gather*}
                \int_{-a}^a f(t)\,dt = 0
            \end{gather*}
        \end{block}
...
\begin{frame}{\glqq{}Hauptsatz\grqq{} der Differential- und Integralrechnung}
    ...
    \begin{columns}
        ...
        Für den Differentenquotienten von $F$ gilt dann
        \small
        \begin{align*}
            \frac{F(x+h)-F(x)}{h} &= \frac1h\left[
            \int_{\color{red}a}^{\color{blue}x+h} f\,dx - {\color{red}\int_a^x} f\,dx\right]
            \\&= \frac1h{\color{blue}\int_x^{x+h}} f\,dx
            \\&= \frac hh f(z)
        \end{align*}
...
Wir gelangen damit zum sogenannten Hauptsatz der Differential- und Integralrechnung: 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    \frametitle<presentation>{\glqq{}Hauptsatz\grqq{} der Differential- und Integralrechnung}
Sei $f$ eine stetige Funktion und $F$ das unbestimmte Integral. Dann ist $F$ differenzierbar und es gilt
    \begin{gather*}
...
\begin{frame}{Stammfunktion}
    ...
    \begin{itemize}
    \item<+-> Die Stammfunktion ist nur bis auf eine Konstante bestimmt, ansonsten ist sie eindeutig. Schreibweise: $F(x)+C$
    \item<+-> Für beliebige Intervalle $[a,b]$ im Definitionsbereich von $f$ gilt:
     \begin{gather*}
         \int_a^b f(t)\,dt = F(b)-F(a).
     \end{gather*}
    \item<+-> Unbestimmtes Integral und Stammfunktion sind eng verwandt,
         \begin{gather*}
             G(x) := \int_a^x f(t)\,dt = F(x) - F(a),
         \end{gather*}
         $G$ ist also die spezielle Stammfunktion mit der Wahl $G(a) = 0$
     \end{itemize}
\end{frame}
...
\begin{frame}{Substitutionsregel 1. Form}
    ...
    \pause
    \begin{block}{}
        Sei $t = \phi(x)$ und daher $\frac{dt}{dx} = \phi'(x)$ mit stetig differenzierbarem $\phi$. Dann gilt
        \begin{gather*}
            \int_a^b f\bigl(\phi(x)\bigr)\phi'(x)\,dx
            = \int_{\phi(a)}^{\phi(b)} f(t)\,dt.
        \end{gather*}
    \end{block}
    \pause
    Formal funktioniert die Substitutionsregel so, dass wir überall $\phi(x)$ durch $t$ ersetzen und formal
    \begin{gather*}
        dt = \frac{dt}{dx}\,dx = \phi'(x)\,dx
    \end{gather*}
    Die Integralgrenzen müssen nun auch in $t = \phi(x)$ ausgedrückt werden.
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Beispiel}
    \begin{gather*}
        \int_a^b x \cos(x^2)\,dx
        \only<3->{=\frac12\int_{a^2}^{b^2} \cos(t)\,dt}
    \end{gather*}
    \pause
    Benutze $\phi(x) = x^2$, dann gilt $\phi'(x) = 2x$
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Substitutionsregel 2. Form}
    \begin{block}{}
        Sei $t = \phi(x)$ und daher $\frac{dt}{dx} = \phi'(x)$ mit stetig differenzierbarer, \textbf{monotoner} Funktion $\phi$. Dann gilt
        \begin{gather*}
            \int_{a}^{b} f(t)\,dt =
            \int_{\phi^{-1}(a)}^{\phi^{-1}(b)} f\bigl(\phi(x)\bigr)\phi'(x)\,dx
            .
        \end{gather*}
    \end{block}
\end{frame}
...
\label{frame:circle}
    ...
    \end{Denkpause}}
    \pause
    Es gilt $1-\cos^2 x = \sin^2 x$, wodurch wir die Wurzel ziehen können. Also probieren wir die Substitution
    \begin{gather*}
    ...
    \begin{multline*}
        \int \sqrt{1-t^2}\,dt = - \int \sqrt{1-\cos^2 x}\sin x\,dx
        \\
        = -\int \sin^2 x\,dx
        = \frac{-1}2\int \bigl(1-\cos (2x)\bigr)\,dx
    \end{multline*}
\end{frame}
...
\begin{frame}
    ...
    \begin{gather*}
        f(x) = \frac{\cos(2x)-1}{2},\qquad F(x) = \frac{-\sin(2x)}4-\frac x2
    \end{gather*}
    \pause
    ...
    \begin{gather*}
        \int_0^1 \sqrt{1-t^2}\,dt = \left[
        \frac{-\sin(2x)}4-\frac x2\right]_{-\frac\pi2}^0
        = -0-0+0+\frac\pi4
    \end{gather*}
\end{frame}
...
\begin{frame}
    ...
    \begin{figure}[H]
      \centering
      \resizebox{0.4\linewidth}{!}{\input{fig/inverse_cuberoot.tikz}}
    \end{figure}
    \begin{itemize}
    ...
    \begin{figure}[H]
      \centering
      \resizebox{0.4\linewidth}{!}{\input{fig/inverse_cuberoot_integrated.tikz}}
    \end{figure}
...
\begin{frame}
    ...
    \begin{figure}[H]
      \centering
      \resizebox{0.4\linewidth}{!}{\input{fig/inverse_linear.tikz}}
    \end{figure}
    \begin{itemize}
    ...
    \begin{figure}[H]
      \centering
      \resizebox{0.4\linewidth}{!}{\input{fig/log.tikz}}
    \end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    \frametitle<presentation>{Warnung}
    ...
    \end{Denkpause}}
    \pause
    ist nicht null, obwohl die Funktion $\tfrac1x$ ungerade ist, da die beiden uneigentlichen Integrale
    \begin{gather*}
...
\begin{frame}{Beispiel: Integral über die Sprungfunktion}
    Sei die Funktion $H$ definiert als
    \begin{gather*}
        H(t) = \begin{cases}
            0 & t<0\\1&t\ge 0
        \end{cases}
...
\mrnote{Im Video sollte man kurz andeuten, wie diese Umformung funktioniert}
...
\begin{itemize}
    ...
    \end{gather*}
    \mrwarning{Wir sollten noch in irgendeiner Form darauf verweisen, dass lediglich die allgemeine Formel zur Berechnung der Fläche eines Trapezes verwendet worden ist. Z.B. auch durch einen kurzen Recherchehinweis}
    \begin{itemize}
...
Das Prinzip hinter der Trapezregel ist, dass sie lineare Polynome exakt integriert und dass bereits mit dieser Anforderung die Formel eindeutig festgelegt ist. Wir wissen bereits, dass eine Gerade in der Ebene durch zwei Punkte $\bigl(x_i,f(x_i)\bigr)$ und $\bigl(x_{i+1},f(x_{i+1})\bigr)$ eindeutig bestimmt ist. Im Folgenden bestimmen wir diese Gerade, betrachten sie als Funktion, und berechnen das Integral. Die Punkte werden interpoliert durch die Gerade
    ...
    \end{gather*}
    \mrwarning{Wird noch etwas genauer im Video darauf eingegangen woher genau die Formel kommt? Ansonsten könnte man das Finden dieser Formel + die Herleitung der restlichen Trapezregel als mehrschrittige Übungsblattaufgabe umformulieren}
...
\begin{frame}{Die Mittelpunktregel}
\begin{itemize}
...
\end{itemize}
\end{frame}
Da hier nur ein Wert der Funktion $f$ zur Verfügung steht, um das Polynom zu bestimmen, müssen wir das Argument, warum die Formel für lineare Polynome exakt ist, abwandeln. Dazu schreiben wir mit beliebigem Faktor $\alpha$:
\mrwarning{Ich würde hier auf jeden Fall nochmal darauf verweisen, dass wir eine beliebige Gerade durch unseren definierten Punkt legen und später zeigen, dass die Steigung tatsächlich irrelevant ist, wir also jede beliebige Gerade wählen können}
\begin{gather*}
    p(t) = f(\overline x_i) + \alpha (t-\overline x_i).
\end{gather*}
Da $t-\overline x_i$ bezüglich $\overline x_i$ eine ungerade Funktion ist, ist ihr Integral null \mrwarning{Zumindest im Video nochmal durch eine ganz kurze Skizze verdeutlichen, dass sich beide Seiten aufheben}. Es gilt also
\begin{gather*}
...
\begin{Aufgabe}
    \mrwarning{Wie genau soll das dann aussehen?}
    \begin{enumerate}
    \item Schreiben Sie eine Funktion, die die Trapezregel mit $n$ Teilintervallen implementiert und den Integralwert zurückgibt. Hierbei soll $n$ ein Funktionsparameter sein, genau wie die rechte und linke Intervallgrenze und die zu integrierende Funktion.
    \item Approximieren Sie mit Ihrer Funktion
    \begin{gather*}
        \int_0^{\pi} \sin(t)\,dt = 2
    \end{gather*}
    mit $n=1,2,4,8,16,32$. Verifizieren Sie die Potenz der Intervalllänge $h=\nicefrac{2\pi}n$ im Fehlerterm der Tabelle.
    \item Machen Sie dasselbe für
    \begin{gather*}
    ...
    \end{enumerate}
\end{Aufgabe}
\gkwarning{Soll ich Gauß-Quadratur auch noch angeben?}
\mrnote{Zumindest als Randnotiz würde ich sie erwähnen}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\gkerror{$\to$ MR: bitte Funktion in R ergänzen oder Zeile löschen}
...
\begin{frame}{Die Fläche beliebiger Kreise}
    \begin{itemize}
        ...
        \begin{align*}
            x &= r \cos t\\
            y &= r \sin t = r\sqrt{1-\cos t} = r \sqrt{1-\left(\tfrac{x}{r}\right)^2}
        \end{align*}
...
\begin{frame}{Das Volumen der Einheitskugel}
    ...
    \end{Denkpause}}
        ...
        \end{column}
        \begin{column}{.5\textwidth}
            \includegraphics[width=\textwidth]{fig/integral-ball.tikz}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \end{gather*}
    \end{itemize}
    \mrnote{Als Übungsblattaufgabe könnte man analog das allgemeine Volumen einer Kugel herleiten lassen}
\end{frame}
